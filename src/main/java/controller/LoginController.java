...
public class LoginController extends HttpServlet{
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) 
		...
		if (user != null && PasswordEncryptor.checkPassword(userPW, user.getPassword())) {
			...
			resp.sendRedirect("/retroescavadeiras/");
		} else resp.sendRedirect("/retroescavadeiras/login.jsp?erro=true");
	}
	
	@Override
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		
		//o false Serve para não criar uma sessão caso ela não exista
		HttpSession session = req.getSession(false);
		
		if(session!=null)
			session.invalidate();
		
		resp.sendRedirect("/retroescavadeiras/login.jsp");
	}
